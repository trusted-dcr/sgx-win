...
void send_command_req(command_req_t req) {
  ...
  std::cout << "[INFO] Sent <COMMAND_REQUEST> to " << daemon_instance->addrs[req.target] << std::endl;
}

void send_command_rsp(command_rsp_t rsp) {
  daemon_instance->async_send(rsp);
  std::cout << "[INFO] Sent <COMMAND_RESPONSE> to " << daemon_instance->addrs[rsp.target] << std::endl;
}

void send_append_req(append_req_t req, entry_t* entries, int size) {
  req.entries = entries;
  daemon_instance->async_send(req);
  std::cout << "[INFO] Sent <APPEND_REQUEST> to " << daemon_instance->addrs[req.target] << std::endl;
}

void send_append_rsp(append_rsp_t rsp) {
  daemon_instance->async_send(rsp);
  std::cout << "[INFO] Sent <APPEND_RESPONSE> to " << daemon_instance->addrs[rsp.target] << std::endl;
}

void send_poll_req(poll_req_t req) {
  daemon_instance->async_send(req);
  std::cout
		<< "[INFO] Sent <POLL_REQUEST, "
		<< req.term << ", "
		<< req.last_term << ", "
		<< req.last_index
		<< "> to "
...
void send_election_req(election_req_t req) {
  daemon_instance->async_send(req);
  std::cout << "[INFO] Sent <ELECTION_REQUEST> to " << daemon_instance->addrs[req.target] << std::endl;
}

void send_election_rsp(election_rsp_t rsp) {
  daemon_instance->async_send(rsp);
  std::cout << "[INFO] Sent <ELECTION_RESPONSE> to " << daemon_instance->addrs[rsp.target] << std::endl;
}

void send_log_req(log_req_t req) {
  daemon_instance->async_send(req);
  std::cout << "[INFO] Sent <LOG_REQUEST> to " << daemon_instance->addrs[req.target] << std::endl;
}

void send_log_rsp(log_rsp_t rsp, entry_t* entries, int size) {
	rsp.entries = entries;
  daemon_instance->async_send(rsp);
  std::cout << "[INFO] Sent <LOG_RESPONSE> to " << daemon_instance->addrs[rsp.target] << std::endl;
}
